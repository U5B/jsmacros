...
function stonkConvertCalculator (ctx) {
  if (lastStonkCoSign.item === '') return logInfo(messages.noStonkCoSign)
  if (lastStonkCoSign.item !== 'Currency Convert') return logInfo('Currency sign not found. Try /stonk buy <amount> or /stonk sell <amount>.')
...
function regularCalculator (ctx) {
  ...
  const currency = ctx.getArg('currency')
  if (!Number.isInteger(itemCount) || !Number.isInteger(price) || !currency) return logInfo('Invalid arguments...')
  const output = calculator(price, itemCount, currency)
