...
function convertToHyper (price, currency) {
  ...
  else if (currency === 'ccs' || currency === 'cxp') price = price / 64
  return price
}
...
function calculator (price, item, currency) { // pain I hardcoded the multiplier
  ...
  if (totalPrice > 0) { // calculate if greater than 0
    ...
    if (totalPrice > 0) { // calculate if greater than 0
      ...
      if (totalPrice > 0) output.standard.count = Math.floor(totalPrice * 8)
    }
  }
  return output
}
